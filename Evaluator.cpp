...
QScriptValue Evaluator::Eval(std::string str) {
    QScriptValue tmp = engine_->evaluate(QString(str.c_str()));
...
void Evaluator::AddAccessor(std::string name, 
    ...
    engine_->globalObject().setProperty(QString(name.c_str()), 
        engine_->newFunction(func), flags);
}

void Evaluator::AddValue(std::string name, QScriptValue val) {
    LCDInfo("AddValue: %s, %f", name.c_str(), (double)val.toNumber());
    engine_->globalObject().setProperty(QString(name.c_str()), val);
}
/*
